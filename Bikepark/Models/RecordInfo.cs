    ...
    public class RecordInfo
    {
        ...
        public static bool Overlap(RecordInfo a, RecordInfo b, int minutesDelay)
        {
            return  a.Status != Models.Status.Closed && 
                    ...
                    (a.Start <= ( (b.Status == Models.Status.Active || b.Status == Models.Status.OnService) ? DateTimeX.Max(b.End ?? DateTime.Now, DateTime.Now).AddMinutes(minutesDelay) : b.End ) ) &&
                    (b.Start <= ( (a.Status == Models.Status.Active || a.Status == Models.Status.OnService) ? DateTimeX.Max(a.End ?? DateTime.Now, DateTime.Now).AddMinutes(minutesDelay) : a.End ) ) ;
        }
